...
local function final_cleanup(doc)
  ...
  for i, block in ipairs(doc.blocks) do
    if block.identifier ~= 'refs' and block.identifier ~= 'bibliography' then
      cleaned_blocks:insert(block)
    else
      print("Removing top-level refs/bibliography div created by external --citeproc")
    end
  end
...
return {
  ...
  {Pandoc = traverse_doc},
  {Div = check_div},
  -- Final cleanup to handle --citeproc running after this filter
  {Pandoc = final_cleanup},
