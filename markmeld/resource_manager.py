...
class ResourceManager:
    ...
    def _discover_all_resources(self):
                ...
                if lua_files:
                    ...
                    if len(lua_files) == 1:
                        ...
                        for lua_file in lua_files:
                            if (subdir.name.replace("-", "_") in lua_file.stem or 
                                subdir.name.replace("-", "") in lua_file.stem):
                                filters[subdir.name] = str(lua_file.absolute())
                                break
                        else:
    ...
    def get_all_resource_variables(self) -> Dict[str, str]:
        ...
        # Add filter variables with new naming convention
...
# Compatibility functions for backward compatibility during migration
def list_filters() -> List[str]:
    """
    List all available filter names.
    
    This function provides backward compatibility during the migration
    from filter_manager to ResourceManager.
...
def inject_resource_variables(target_dict: Dict) -> Dict:
    ...
    This function adds all available resource variables (filters, templates, CSL files)
    to the provided dictionary, making them available for variable substitution.
