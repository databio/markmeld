...
class GoogleDriveProcessor:
    ...
    def __init__(
        ...
        if credentials_dict is not None:
            ...
            try:
                ...
                self._credentials_source = (
                    f"MM_GOOGLE_DRIVE_CREDENTIALS env var (file: {env_creds})"
                )
    ...
    def _log_credentials_info(self) -> None:
        ...
        _LOGGER.info(
            f"Google Drive Processor initialized: {self._credentials_info.get('client_email', 'unknown')} ({self._credentials_info.get('project_id', 'unknown')})"
        )
    ...
    def update_file(
        ...
        media_body = MediaIoBaseUpload(
            io.BytesIO(content_bytes), mimetype=mime_type, resumable=True
        )
    ...
    def download_doc(
        ...
        if update_figure_paths:
            markdown_content = self._update_document_figure_paths(
                markdown_content, doc_id, folder_id
            )
    ...
    def _download_raw_markdown(
        ...
        if doc_path.exists():
            _LOGGER.info(
                "✗ Doc changed since last build - downloading fresh content from Google Drive..."
            )
        else:
            _LOGGER.info("⬇ No cached copy yet - downloading content from Google Drive...")
    ...
    def _download_first_tab(self, doc_id: str) -> str:
        ...
        _LOGGER.info("Fetching document via Docs API (first tab only)...")
    ...
    def _download_with_changes(self, doc_id: str) -> str:
        ...
        _LOGGER.info("Fetching document via Docs API with inline suggestions...")
        doc = (
    ...
    def _document_has_suggestions(self, doc_id: str) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "Ensure the Google Docs API is enabled and documents.readonly scope is included."
            )
            return False
                        ...
                        try:
                            ...
                            if created:
                                try:
                                    ...
                                    created_str = (
                                        created[:10] if len(created) >= 10 else "Unknown date"
                                    )
                            else:
    ...
    def _document_has_changed(self, doc_id: str) -> bool:
        ...
        if not stored_mtime:
            ...
            _LOGGER.info(
                "  No stored modifiedTime - treating as changed (first build or cleared cache)"
            )
            return True
        ...
        try:
            ...
            _LOGGER.warning(
                f"  Could not fetch modifiedTime ({str(e)[:100]}) - assuming changed (fail open)"
            )
            return True
        ...
        if changed:
            _LOGGER.info(
                f"  modifiedTime changed: stored {stored_mtime} -> live {live_mtime} - will re-download"
            )
        else:
    ...
    def _get_doc_path(self, doc_id: str, changed: bool = False) -> Path:
        ...
        return self.cache_manager.get_cache_path(
            doc_id, "docs", self._get_doc_filename(doc_id, changed=changed)
        )

    def _load_from_disk(self, doc_id: str, changed: bool = False) -> str | None:
        ...
        doc_path = self.cache_manager.get_cache_path(
            doc_id, "docs", self._get_doc_filename(doc_id, changed=changed)
        )
        ...
        try:
            ...
            try:
                ...
                if modified_time:
                    ...
                    _LOGGER.warning(
                        "  No modifiedTime from Google Drive API - cache invalidation may not work correctly"
                    )
    ...
    def load_local_digest(self, file_identifier: str, doc_id: str) -> str | None:
    ...
    def _resolve_folder_id(self, doc_id: str, folder_id: str | None = None) -> str | None:
    ...
    def _log_figure_processing_summary(self, results: dict[str, Any]) -> None:
        ...
        if results["failed"]:
            ...
            _LOGGER.warning("   Check the error messages above for details on each failure.")
        ...
        if figure_type == "svg":
            ...
            if success:
                ...
                if file_info and "md5Checksum" in file_info:
                    self.figure_converter.save_digest(
                        fig_path, file_info["md5Checksum"], doc_id, "file"
                    )
                ...
                _LOGGER.warning("    Check inkscape installation and SVG file validity")
            ...
            if converted_path:
                ...
                _LOGGER.warning("    Check table_pdf_builder installation")
    ...
    def process_document_figures(
        ...
        for reference_order, (fig_path, params) in enumerate(figure_data, start=1):
            ...
            if figure_type == "sheet":
                ...
                if converted_path:
                    ...
                    _LOGGER.error("  Failed to convert Google Sheet")
            ...
            if not file_info:
                _LOGGER.error("  Failed: File not found in Google Drive")
                results["failed"].append(fig_path)
                continue
            ...
            try:
                ...
                if not self.figure_converter.needs_conversion(
                    fig_path, doc_id, output_path, file_info, params
                ):
                    ...
                    _LOGGER.info(
                        f"  Checking source file: {source_file_path} exists={source_file_path.exists()}"
                    )
                ...
                source_file = self._get_cached_file_or_download(
                    fig_path,
                    file_info,
                    doc_id,
                    figure_type,
                ...
                # Convert file
                converted_path = self._convert_file(
                    fig_path, source_file, file_info, params, doc_id, figure_type, output_path
                )
        ...
        if remaining_svgs:
            ...
            if unreplaced_from_failures:
                _LOGGER.warning(
                    f"WARNING: {len(unreplaced_from_failures)} SVG path(s) not replaced due to conversion failures:"
                )
            ...
            if unreplaced_unknown:
                _LOGGER.warning(
                    f"WARNING: {len(unreplaced_unknown)} SVG path(s) not replaced for unknown reasons:"
                )
    ...
    def process_document_csvs(
        ...
        doc_content = self.download_doc(
            doc_id, clean=False, parse_frontmatter=False, update_figure_paths=False
        )
        ...
        return {"csvs": csv_paths, "results": results}
    ...
    def _process_csv_files(
        ...
        for csv_path in csv_paths:
            ...
            if not file_info:
                _LOGGER.error("  Failed: CSV file not found in Google Drive")
            ...
            if skip_unchanged and remote_digest:
                local_digest = (
                    self.load_local_digest(csv_path, doc_id)
                    if doc_id
                    else self.load_local_digest(csv_path, "default")
                )
            ...
            try:
                ...
                if skip_unchanged and remote_digest:
                    self.save_local_digest(
                        csv_path, remote_digest, doc_id
                    ) if doc_id else self.save_local_digest(csv_path, remote_digest, "default")
    ...
    def _find_subfolder_by_name(self, parent_folder_id: str, subfolder_name: str) -> str | None:
    ...
    def _update_document_figure_paths(
        self, markdown_content: str, doc_id: str, folder_id: str | None = None
    ) -> str:
    ...
    def process_document_assets(
        ...
        try:
            ...
            # Update summary
            results["summary"]["total_figures"] = (
                len(figure_results["results"].get("processed", []))
                + len(figure_results["results"].get("skipped", []))
                + len(figure_results["results"].get("failed", []))
            )
            results["summary"]["figures_processed"] = len(
                figure_results["results"].get("processed", [])
            )
            results["summary"]["figures_skipped"] = len(
                figure_results["results"].get("skipped", [])
            )
